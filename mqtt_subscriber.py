...
class Mqtt_subscriber(NeuronModule):
    def __init__(self, **kwargs):
        super(Mqtt_subscriber, self).__init__(**kwargs)

        logger.debug("[mqtt_subscriber] neuron called with parameters: %s" % kwargs)
        ...
        if not self._is_parameters_ok():
            logger.debug("[mqtt_subscriber] One or more invalid parameters, neuron will not be launched")
        else:
            ...
            if self.username is not None and self.password is not None:
                logger.debug("[mqtt_subscriber] Username and password are set")
                self.client.username_pw_set(self.username, self.password)

            if self.ca_cert is not None and self.certfile is not None and self.keyfile is not None:
                logger.debug("[mqtt_subscriber] Active TLS with client certificate authentication")
                ...
                logger.debug("[mqtt_subscriber] Active TLS with server CA certificate only")
                self.client.tls_set(ca_certs=self.ca_cert)
                self.client.tls_insecure_set(self.tls_insecure)

            try:
                self.client.connect(self.broker_ip, port=self.port, keepalive=self.keepalive)
                self.client.subscribe(topic=self.topic, payload=self.payload, qos=int(self.qos)
                logger.debug("[mqtt_subscriber] Message subscribed to topic %s: %s" % (self.topic, self.payload))
                self.client.disconnect()
            except socket.error:
                logger.debug("[mqtt_subscriber] Unable to connect to broker %s" % self.broker_ip)

    def _is_parameters_ok(self):
        if self.broker_ip is None:
            print("[mqtt_subscriber] ERROR: broker_ip is not set")
            return False
        ...
        if self.port is not None:
            if not isinstance(self.port, int):
                try:
                    ...
                    print("[mqtt_subscriber] ERROR: port must be an integer")
                    return False

        if self.topic is None:
            print("[mqtt_subscriber] ERROR: topic is not set")
            return False

        if self.payload is None:
            print("[mqtt_subscriber] ERROR: payload is not set")
            return False
        ...
        if self.qos:
            if not isinstance(self.qos, int):
                try:
                    ...
                    print("[mqtt_subscriber] ERROR: qos must be an integer")
                    return False
            if self.qos not in [0, 1, 2]:
                print("[mqtt_subscriber] ERROR: qos must be 0,1 or 2")
                return False
        ...
        if self.keepalive:
            if not isinstance(self.keepalive, int):
                try:
                    ...
                    print("[mqtt_subscriber] ERROR: keepalive must be an integer")
                    return False

        if self.username is not None and self.password is None:
            print("[mqtt_subscriber] ERROR: password must be set when using username")
            return False
        if self.username is None and self.password is not None:
            print("[mqtt_subscriber] ERROR: username must be set when using password")
            return False

        if self.protocol:
            if self.protocol not in ["MQTTv31", "MQTTv311"]:
                print("[mqtt_subscriber] Invalid protocol value, fallback to MQTTv311")
        ...
        if self.certfile is not None and self.keyfile is None:
            print("[mqtt_subscriber] ERROR: keyfile must be set when using certfile")
            return False
        if self.certfile is None and self.keyfile is not None:
            print("[mqtt_subscriber] ERROR: certfile must be set when using keyfile")
            return False

        if self.certfile is not None and self.keyfile is not None:
            if self.ca_cert is None:
                print("[mqtt_subscriber] ERROR: ca_cert must be set when using keyfile and certfile")
                return False
